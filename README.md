<a id="top"></a>

<div align="center">
  <img src="https://raw.githubusercontent.com/Vittorio-Francesco-Ardente/GluGluPop_TPS_API/design/Logos/Asset_ReadMe_HD.png" alt="GluGluPop Logo" width="400">
</div>

# 🎬 GluGluPop  
### Swipe. Match. Watch. Eat.

**GluGluPop** è un'app di intrattenimento che rivoluziona il modo di scoprire film e serie TV, trasformando l'indecisione in un'esperienza rapida, visiva e coinvolgente.  
Ispirata ai meccanismi di swipe di TikTok e Tinder, GluGluPop unisce **cinema, raccomandazioni intelligenti e abbinamenti food & drink** in un'unica piattaforma moderna.

---

## 🚀 Vision & Concept
GluGluPop non è un semplice catalogo di film.  
È un **discovery engine** progettato per:

- Ridurre il tempo perso a scegliere cosa guardare  
- Offrire suggerimenti personalizzati e dinamici  
- Trasformare la visione in un'esperienza completa (film + cibo + drink)  
- Favorire la scelta condivisa grazie alla **modalità gruppo**

🎯 **Obiettivo**: rendere la scoperta dell'intrattenimento semplice, divertente e memorabile.

[⬆ Back to top](#top)

---

## ✨ Funzionalità Principali

- 🔄 **Swipe Experience**  
  Trailer a schermo intero con like / skip immediato

- 🧠 **Raccomandazioni Intelligenti**  
  Algoritmi basati sui gusti e sulle interazioni dell'utente

- 👥 **Modalità Gruppo**  
  Decisioni condivise per serate cinema tra amici

- 🍿 **Cinema + Food**  
  Abbinamenti automatici con snack, cibo e drink

- 📱 **PWA Installabile**  
  Utilizzabile come una vera app mobile

- 💾 **Offline Ready**  
  Cache e storage locale per una UX fluida

[⬆ Back to top](#top)

---

## 🧩 API Utilizzate

- **TMDb API** – catalogo film e serie TV  
- **YouTube API** – trailer ufficiali  
- **JustWatch API** – disponibilità sulle piattaforme streaming  
- **TasteDive API** – suggerimenti per affinità di contenuti  
- **Spoonacular API** – abbinamenti cibo  
- **TheCocktailDB API** – suggerimenti drink  

[⬆ Back to top](#top)

---

## 🛠️ Tecnologie Utilizzate

### Frontend
- **React + Vite**
- **TailwindCSS**
- **Swiper.js** (gestione swipe)
- **Zustand** (state management)

### Storage & Performance
- **LocalStorage**
- **IndexedDB**

### Deploy
- **Vercel**
- **Progressive Web App (PWA)**

[⬆ Back to top](#top)

---

## 🎨 UI / UX & Design

- Design **mobile-first**
- Animazioni fluide e feedback immediato
- Iconografia e branding ispirati ai grandi player dell'intrattenimento
- Interfaccia moderna, intuitiva e orientata all'esperienza utente

Il progetto presta particolare attenzione a:
- leggibilità
- coerenza visiva
- percezione di qualità e affidabilità

[⬆ Back to top](#top)

---

## 📬 Contatti

Per informazioni, collaborazioni o chiarimenti:

- 📧 ardente.vittorio.07@itisdalmine.edu.it  
- 📧 previtali.mattia.07@itisdalmine.edu.it  
- 📧 decano.niccolo.07@itisdalmine.edu.it  
- 📧 gamba.alessandro.07@itisdalmine.edu.it  

[⬆ Back to top](#top)

---

## 📄 Stato del Progetto

🟡 **In sviluppo**  
Il progetto è attualmente in fase di progettazione avanzata e sviluppo frontend.

Prossimi step:
- completamento UI definitiva  
- ottimizzazione algoritmo di raccomandazione  
- testing avanzato  
- deploy PWA finale  

[⬆ Back to top](#top)

---

## ⭐ Perché GluGluPop

> Non scegliere cosa guardare.  
> **Scoprilo. Condividilo. Vivilo.**

GluGluPop è pensato per utenti, non per cataloghi.  
Ed è progettato per sembrare — e comportarsi — come un vero prodotto commerciale.

[⬆ Back to top](#top)
